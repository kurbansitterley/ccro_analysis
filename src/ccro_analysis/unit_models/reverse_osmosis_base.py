...
from ccro_analysis.costing.reverse_osmosis import cost_reverse_osmosis
...
class ReverseOsmosisBaseData(InitializationMixin, UnitModelBlockData):
    ...
    def initialize_build(
        ...
        if use_interval_initializer:
            init_log.info(
                "Starting unit initialization WITH interval arithmetic pre-solve..."
            )
            interval_initializer(self)
        else:
            init_log.info(
                "Starting unit initialization WITHOUT interval arithmetic pre-solve..."
            )
