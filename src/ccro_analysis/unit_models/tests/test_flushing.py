...
from ccro_analysis.unit_models.flushing import Flushing
...
def test_solve(build):
    ...
    assert pytest.approx(flushing_efficiency, rel=1e-5) == value(
        m.fs.unit.flushing_efficiency
    )
    assert pytest.approx(post_flushing_concentration, rel=1e-5) == value(
        m.fs.unit.post_flushing_concentration
    )
...
def test_solve_with_test_data(build_with_test_data):
    ...
    assert pytest.approx(mean_residence_time, rel=1e-5) == value(
        m.fs.unit.mean_residence_time
    )
